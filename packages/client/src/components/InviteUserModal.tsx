...
export function InviteUserModal({ matchId, onClose }: Props) {
  const invite = useMutation({
    mutationFn: (email: string) => matchesApi.invite(matchId, email),
    onSuccess: () => {
      form.reset()
    },
  })
  ...
  const form = useForm({
    defaultValues: { email: '' },
    validators: { onChange: inviteEmailSchema },
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 px-4">
      <div className="bg-white border border-gray-200 dark:bg-green-950 dark:border-white/10 rounded-2xl p-6 w-full max-w-md shadow-lg">
        <h3 className="text-gray-900 dark:text-white font-bold text-lg mb-4">Invite a user</h3>
        <form
          onSubmit={(e) => {
            e.preventDefault()
            e.stopPropagation()
        ...
        >
          <form.Field
            name="email"
            validators={{ onChange: inviteEmailSchema.shape.email }}
          >
            {(field) => (
          ...
          {invite.isSuccess && (
            <p className="text-sm text-emerald-600 dark:text-emerald-400">Invitation sent!</p>
          )}
          {invite.error && (
            <p className="text-sm text-red-500 dark:text-red-400">
              {(invite.error as AxiosError<{ error: string }>).response?.data?.error ??
